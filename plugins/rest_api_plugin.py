...
class REST_API(get_baseview()):
        ...
        if not (pause and unpause):
            if pause or unpause:
                try:
                    ...
                    warning = "Failed to set the state (pause, unpause) of the DAG: " + str(
                        e)
    ...
    def refresh_dag(self, base_response):
        ...
        try:
            ...
            error_message = "An error occurred while trying to Refresh the DAG '" + \
                str(dag_id) + "': " + str(e)
    ...
    def refresh_all_dags(self, base_response):
        ...
        try:
            if (versiontuple(str(airflow.__version__)) < versiontuple("1.10.0")):
                from airflow.www.views import Airflow
                refresh_result = Airflow().refresh_all()
            else:
                all_dags = self.get_dagbag()
                print(all_dags)
                logging.info("Refresh Result: " + str(all_dags))
                output = str(all_dags.dags)
                temp_args = request.args.to_dict()

                for dag_id in all_dags.dags:
                    temp_args['dag_id'] = dag_id
                    ...
                    try:
                        ...
                        refresh_result = Airflow().refresh()
                        logging.info("Refresh Result: " + str(refresh_result))
                        logging.info("DAG [{}] is now fresh as a daisy;".format(dag_id))
                    except Exception as e:
                        error_message = "An error occurred while trying to Refresh the DAG '" + \
                            str(dag_id) + "': " + str(e)
                        logging.error(error_message)
            ...
            error_message = "An error occurred while trying to Refresh all the DAGs: " + \
                str(e)
            logging.error(error_message)
            return REST_API_Response_Util.get_500_error_response(base_response, error_message)
    ...
    def execute_cli_command(airflow_cmd_split):
        ...
        process = subprocess.Popen(
            airflow_cmd_split, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        process.wait()
...
if rbac_authentication_enabled == True:
    rest_api_view = {"category": "Admin",
                     "name": "REST API Plugin",  "view": REST_API()}
else:
